    ...
    int minimumSum(vector<vector<int>>& a) 
    {
        ...
        for(int i=0;i<n;i++)
        {
            for(int j=0;j<m;j++)
            {
                ...
                int b1=helper(0,i,0,j,a);
                int b2=helper(i+1,n-1,0,j,a);
                int b3=helper(0,n-1,j+1,m-1,a);
                ans=min(ans,b1+b2+b3);

                int c1=helper(0,i,0,m-1,a);
                int c2=helper(i+1,n-1,0,j,a);
                int c3=helper(i+1,n-1,j+1,m-1,a);
                ans=min(ans,c1+c2+c3);

                int d1=helper(0,i,0,j,a);
                int d2=helper(0,i,j+1,m-1,a);
                int d3=helper(i+1,n-1,0,m-1,a);
